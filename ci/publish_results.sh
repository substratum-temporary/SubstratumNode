...
curl https://raw.githubusercontent.com/substratum-temporary/SubstratumNode-results/master/README.md > README.md.old
if [[ "$TRAVIS_PULL_REQUEST_SLUG" == "" ]]; then
  RESULTS_LABEL="$TRAVIS_BRANCH"
else
  RESULTS_LABEL="$TRAVIS_PULL_REQUEST_SLUG"
fi
NEW_LINE="* $(date -u) - $RESULTS_LABEL - $STATUS: [generated.zip](/results/$RESULTS_LABEL/generated.zip)"
...
mv generated.zip "/results/$RESULTS_LABEL"
